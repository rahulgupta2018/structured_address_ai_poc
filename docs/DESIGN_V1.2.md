...
CSV Batch (from GCS) or API Request
  ...
  │     ├─ Async concurrent requests (semaphore-bounded, ~50/worker)
  │     ├─ Circuit breaker pattern (fail-fast if service degraded)
  │     ├─ Rate limiter respects provider RPM quota
  │     ├─ Enriched prompt with postal code context + mismatch warnings
  │     ├─ Per-request timeout (10s) + retry with exponential backoff
...
| Step 6 | Local Ollama, 4 threads (ThreadPool) | Managed LLM (Vertex AI), async semaphore (50 concurrent/worker), circuit breaker, rate limiter | 50x throughput per worker; backpressure prevents provider overload |
